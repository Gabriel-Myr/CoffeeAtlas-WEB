...
- `apps/api/lib/server/api-primitives.ts` -> `HttpError`, `badRequest`, `notFound`, `conflict`
- `apps/api/lib/server/api-helpers.ts` -> `apiSuccess`, `apiError`
...
- `apps/api/lib/server/auth-user.ts` -> 未登录时抛 `HttpError(401, 'unauthorized', ...)`
- `apps/api/lib/server/wechat-auth.ts` -> 微信配置缺失、微信接口失败、登录失败分别给不同 code
