...
- `apps/api/lib/catalog.ts`
- `apps/api/lib/server/public-api.ts`
- `apps/api/lib/server/favorites-api.ts`
- `apps/api/lib/server/admin-catalog.ts`
- `apps/api/lib/supabase.ts`
- `apps/api/db/sql/**`
- `apps/api/db/migrations/**`
- `apps/api/scripts/import-*.ts`
...
- `apps/api/lib/catalog.ts` -> `fetchBeanContext()`
- `apps/api/lib/server/favorites-api.ts` -> `hydrateFavorites()`
...
- `apps/api/db/sql/**` - 基础 schema、索引、RLS、视图、seed
- `apps/api/db/migrations/**` - 后续应用级增量修改

规则：

- 新的增量改动优先追加到 `apps/api/db/migrations/**`
- 不要重写已经被环境使用过的历史迁移
