...
- `apps/api/lib/catalog.ts` -> `RoasterBeanRow`, `BeanRow`, `RoasterRow`
- `apps/api/lib/server/auth-jwt.ts` -> `JwtPayload`
- `apps/api/lib/server/admin-catalog.ts` -> `CreateAdminBeanInput`
...
- `NextRequest`, `NextResponse` 只留在 `apps/api/**`
