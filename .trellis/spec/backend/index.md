...
> 适用于 `apps/web/app/api/**`、`apps/web/lib/server/**`、`apps/web/lib/catalog.ts`、`apps/web/lib/supabase.ts`、`apps/web/scripts/**` 和 `apps/web/db/**`。

---

## Backend Surface In This Repo

当前后端主要位于 `apps/web`，分成 4 类入口：
...
- `packages/api-client` 和 `packages/domain` 仍是部分骨架，当前运行时主路径仍在 `apps/web` 与 `apps/miniprogram/src/services/api.ts`。
- 公开 catalog 读取支持无 Supabase 环境时 fallback 到 `apps/web/lib/sample-data.ts`；写操作、鉴权、收藏、微信登录不允许静默 fallback。
- `app/api/v1/**` 已经存在；不要再把它当作“待开发目录”。
- `apps/web/lib/server/admin-auth.ts` 仍是占位实现，新增管理端写接口时要显式标明风险，不要误写成已完成鉴权。
