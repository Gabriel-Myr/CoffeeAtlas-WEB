...
> 适用于 `apps/miniprogram`。默认把这里当成当前任务主规范；只有在联动 API、shared-types 或 Web 后端时，再补读其他 package 的 spec。
...
- 改页面布局、卡片、筛选条、空态 -> `component-guidelines.md` + `quality-guidelines.md`
...
- 如果改动同时影响 `/api/v1/*`、`packages/shared-types` 或 Web server mapping，再补读 `.trellis/spec/web/backend/` 与 `.trellis/spec/guides/`。
