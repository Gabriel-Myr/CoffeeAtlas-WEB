...
- **技术栈**: Next.js 16 Route Handlers + React 19 + Taro 3.x + Supabase + TypeScript

## 核心能力

### 1. v1 API
- 主 API 位于 `apps/api/app/api/v1/**`
...
### 3. 数据访问与导入
- `apps/api/lib/catalog.ts` 负责目录读取和 sample fallback
- `apps/api/lib/server/*` 负责服务端 API 组装、鉴权、收藏等逻辑
- `apps/api/scripts/import-*.ts` 负责导入
- `apps/api/db/sql/**` + `apps/api/db/migrations/**` 管理 schema 与增量变更
...
- `apps/api` -> Next.js API、Supabase server access、导入与 smoke 脚本
...
- `apps/api/app/api/beans` 和 `apps/api/app/api/roasters` 仍是 legacy 兼容层。
